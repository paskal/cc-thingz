...
- exec: `customize-file.sh` copies a bundled prompt or agent file into a project-level or user-level override path on demand, so only files someone intends to edit become overrides. It refuses to overwrite an existing override, including a dangling symlink that would otherwise be followed outside the override directory, and rejects absolute or traversing paths. It also refuses to write through a symlinked directory component: `mkdir -p` accepts an ancestor symlink and `cp` follows it, so a repository shipping a `.claude/exec-plan/prompts` symlink could otherwise redirect the copy wherever it pointed. Components at or below the override root are checked; the data dir itself is not, since a symlinked `$HOME` or `~/.claude` is a legitimate setup. A passed-but-empty data directory, and a data dir of `/`, are reported rather than silently downgraded to a project-level copy
...
- exec: `external_review_cmd` must be a single line. A multi-line value is reported rather than run, since the single `read` that splits it into command and flags only ever saw the first line
...
- exec: `codex-review.md` no longer carries launcher or availability instructions of its own. It named `run-codex.sh` as the command to run and said to skip the phase when codex is not installed, both of which now live in step 9 of the exec skill and would contradict it; the file defers to step 9 and keeps only the prompt
...
- the README gained an **External review contract** section stating what a custom command must do, emit and leave untouched, and `run-external-review.sh`'s header points at it rather than restating it. Two requirements there are easy to miss: the tool has to be able to run shell commands and read files in the working tree, since the prompt tells it to run a diff and read the plan, the progress file and sources - a plain text-in/text-out LLM CLI satisfies every other requirement and reviews nothing - and it must read nothing from stdin, which the script redirects from `/dev/null`
- `references/usage.md` no longer describes the `SessionStart` hook as the way to customize prompts. It points at the README's Customization section, which carries the runnable form with both plugin paths spelled out
