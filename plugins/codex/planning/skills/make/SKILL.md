...
- **show rules**: run `bash <plugin-root>/scripts/resolve-rules.sh planning-rules.md` and display the output. if the output is empty, tell the user no custom rules are configured at either level. otherwise, to determine the source, check if `.codex/planning-rules.md` exists and is non-empty (project-level) — if not, the output came from user-level. tell the user which level it came from.
- **add/update project rules**: write content to `.codex/planning-rules.md` in the current working directory.
- **add/update user rules**: write content to `${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md`, creating its parent directory first.
- **clear project rules**: delete `.codex/planning-rules.md`.
- **clear user rules**: delete `${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md` if it exists.

project-level rules (`.codex/planning-rules.md`) take precedence over user-level rules (`${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md`). when both non-empty files exist, only project-level rules are loaded. empty files are treated as absent and fall through to the next level. see `<plugin-root>/references/custom-rules.md` for full documentation on the rules mechanism.

**CRITICAL: this skill must NEVER modify its own files (skills, scripts, references, hooks, plugin.json). the ONLY files it may create or modify for rules management are `.codex/planning-rules.md` and `${CODEX_HOME:-$HOME/.codex}/cc-thingz/planning/planning-rules.md`. if the user asks to change the skill's behavior, create a plan for it — do not edit skill files directly.**
...
ask the user to select the preferred approach before creating the plan.
...
- **Interactive review**: check if `revdiff` is installed (`which revdiff`).
  - **if revdiff is available**: run `<plugin-root>/scripts/launch-plan-review.sh <plan-file-path>`.
    ...
    3. run `<plugin-root>/scripts/launch-plan-review.sh <plan-file-path>`
    4. repeat until no output (user quit without annotations)
  - **if revdiff is not available**: fall back to `<plugin-root>/scripts/plan-annotate.py <plan-file-path>`.
    ...
    2. revise the plan file to address each annotation
    3. run `<plugin-root>/scripts/plan-annotate.py <plan-file-path>`
